...
public partial class AdminPages : System.Web.UI.Page {
    ...
    private void btnUploadFile_Click(object sender, EventArgs e) {
        ...
        string a = "<p><a href=\"{0}ThonHttpHandlers/File.ashx?filepath={1}\">{2}</a></p>";
        string text = txtUploadFile.FileName + " (" + SizeFormat(txtUploadFile.FileBytes.Length, "N") + ")";
        txtContent.Text += string.Format(a, HelperUtilities.InternetAppRoot, BlogSettings.Instance.StorageLocation + "Files" + Path.DirectorySeparatorChar + Server.UrlEncode(txtUploadFile.FileName), text);
    }
    ...
    private void btnSave_Click(object sender, EventArgs e) {
        ...
        if (Request.QueryString["id"] != null)
            page = Thon.ZaszBlog.Support.CodedRepresentations.Page.GetPage(new Guid(Request.QueryString["id"]));
        else
            page = new Page();
        ...
        if (cbIsFrontPage.Checked) {
            foreach (Page otherPage in Thon.ZaszBlog.Support.CodedRepresentations.Page.Pages)
            {
    ...
    private void BindPage(Guid pageId) {
        Page page = Thon.ZaszBlog.Support.CodedRepresentations.Page.GetPage(pageId);
    ...
    private void BindParents(Guid pageId) {
        ...
        ddlParent.Items.Insert(0, "-- No Parent --");
        if (pageId != Guid.Empty) {
            Page parent = Thon.ZaszBlog.Support.CodedRepresentations.Page.GetPage(pageId);
            if (parent != null)
