...
namespace Thon.ZaszBlog.Support.Web.HttpHandlers
{
    ...
    /// The QueryString will contain only one of these : category, post, author
	/// </remarks>
	...
	public class SyndicationHandler : IHttpHandler
	{
		...
		public void ProcessRequest(HttpContext context)
		{
			...
			List<IShowed> list = GenerateItemList(context);
			list = CleanList(list); // Filters all the not visible comments , posts .
        ...
        /// Gets all the posts for a category or author or a particular post or comments
		/// </summary>
		private static List<IShowed> GenerateItemList(HttpContext context)
		{
			if (!string.IsNullOrEmpty(context.Request.QueryString["category"]))
			{
                ...
                return Post.GetPostsByCategory(categoryId).ConvertAll(new Converter<Post, IShowed>(ConvertToIShowed));
			}
			...
			if (!string.IsNullOrEmpty(context.Request.QueryString["author"]))
			{
                ...
                return Post.GetPostsByAuthor(author).ConvertAll(new Converter<Post, IShowed>(ConvertToIShowed));
			}
			...
			if (!string.IsNullOrEmpty(context.Request.QueryString["post"]))
			{
                ...
                return post.Comments.ConvertAll(new Converter<Comment, IShowed>(ConvertToIShowed));
			}
			...
			// The latest posts
            return Post.Posts.ConvertAll(new Converter<Post, IShowed>(ConvertToIShowed));
		}

		private static List<IShowed> CleanList(List<IShowed> list)
		{
			return list.FindAll(delegate(IShowed item)
			{
		...
		/// </summary>
		private static List<IShowed> RecentComments()
		{
			...
			temp.Sort(); // Sorts by date created becoz we implemented IComparable interface for the comments
			temp.Reverse(); // Latest comment comes first.
            ...
            return list.ConvertAll(new Converter<Comment, IShowed>(ConvertToIShowed));
		}
        ...
        /// <summary>
        /// Retrieves the request from the QueryString.
        /// </summary>
        /// <param name="context">An <see cref="T:System.Web.HttpContext"></see> object that provides references 
        /// to the intrinsic server objects (for example, Request, Response, Session, and Server) used to service HTTP requests.
        /// </param>
        /// <returns>
        /// A string of the format title-subtitle where subtitle is the name of the catgory, author or the 
        /// post to which the user has subscribed the feed.
        /// </returns>
		private static string RetrieveTitle(HttpContext context)
		{
            ...
            // Feeds are generated only for categories , posts , and authors .. a feed for each comment
			if (!string.IsNullOrEmpty(context.Request.QueryString["category"]))
			{
				Guid categoryId = new Guid(context.Request.QueryString["category"]);
		...
		/// <param name="items">The collection of <see cref="IShowed"/> instances used when setting the response header details.</param>
		...
		private static void SetHeaderInformation(HttpContext context, List<IShowed> items, SyndicationFormat format)
		{
