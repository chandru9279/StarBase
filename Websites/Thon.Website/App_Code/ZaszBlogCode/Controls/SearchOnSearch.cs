...
namespace Thon.ZaszBlog.Controls
{
  ...
  public class SearchOnSearch : Control
  {
    ...
    private string Html()
    {
      if (Context.Request.UrlReferrer != null && !Context.Request.UrlReferrer.ToString().Contains(SupportUtilities.AbsoluteWebRoot.ToString()) && IsSearch)
      {
    ...
    /// </summary>
    private string WriteHtml(List<IShowed> items, string searchTerm)
    {
