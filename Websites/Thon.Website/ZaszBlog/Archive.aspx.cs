...
public partial class ArchiveAspx : BlogBasePage
{
  ...
  private void CreateArchive()
  {
    foreach (Category cat in Category.Categories)
    {
      ...
      feed.HRef = SupportUtilities.RelativeWebRoot + "ZaszBlogHttpHandlers/Syndication.ashx?category=" + cat.Id.ToString();
      ...
      h2.Attributes["id"] = SupportUtilities.RemoveIllegalCharacters(name);
  ...
  private void AddTotals()
  {
    ...
    ltPosts.Text = Post.Posts.Count + " posts";
    if (BlogSettings.Instance.IsCommentsEnabled)
        ltComments.Text = comments + " comments";

    if (BlogSettings.Instance.EnableRating)
        ltRaters.Text = raters + " raters";
  }
}
