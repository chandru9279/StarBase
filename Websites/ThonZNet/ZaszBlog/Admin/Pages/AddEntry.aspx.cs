...
public partial class AdminAddEntry : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
{
	...
	private void btnUploadImage_Click(object sender, EventArgs e)
	{
		...
		string path = HelperUtilities.InternetAppRoot.ToString();
        string img = string.Format("<img src=\"{0}ThonHttpHandlers/Image.ashx?picturepath={1}\" alt=\"\" />", path, BlogSettings.Instance.StorageLocation + "Files" + Path.DirectorySeparatorChar + Server.UrlEncode(txtUploadImage.FileName));
	...
	private void btnUploadFile_Click(object sender, EventArgs e)
	{
        ...
        string a = "<p><a href=\"{0}ThonHttpHandlers/File.ashx?filepath={1}\" rel=\"enclosure\">{2}</a></p>";
		string text = txtUploadFile.FileName + " (" + SizeFormat(txtUploadFile.FileBytes.Length, "N") + ")";
        txtContent.Text += string.Format(a, HelperUtilities.InternetAppRoot.ToString(), BlogSettings.Instance.StorageLocation + "Files" + Path.DirectorySeparatorChar + Server.UrlEncode(txtUploadFile.FileName), text);
	}
	...
	private void btnSave_Click(object sender, EventArgs e)
	{
		...
		post.DateCreated = DateTime.ParseExact(txtDate.Text, "yyyy-MM-dd HH:mm", null);
	...
	private void BindPost(Guid postId)
	{
		...
		txtSlug.Text = SupportUtilities.RemoveIllegalCharacters(post.Slug);
	...
	private void BindDrafts()
	{
		...
		if (counter > 0)
		{
            ...
            aDrafts.InnerHtml = string.Format("There are {0} unpublished post(s)", counter);
		}
	}
	...
	public void RaiseCallbackEvent(string eventArgument)
	{
		...
		{
			_Callback = SupportUtilities.RemoveIllegalCharacters(eventArgument.Trim());
		}
	}
